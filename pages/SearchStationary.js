...
const SearchPlaces=({route})=>{
    ...
    ctx.items.forEach(element => {
        if((element.name.toUpperCase().includes(text.toUpperCase().trim().replace(/\s/g, "")) && (element.isFood ===false))){
            console.log(element.isFood === false)
            res.push(element)
        }
    });
    console.log(res)
        ...
        <View>
            ...
            <View style={styles.tabs}>
                        <ScrollView horizontal={true}>
                            ...
                            <TouchableOpacity style={styles.tab2} onPress={()=>{navigation.navigate("SearchFood",{tex: {text}});}}>
                                <Text style={styles.tabtext2}>Food</Text>
                            </TouchableOpacity>
                            <TouchableOpacity style={styles.tab1} onPress={()=>{navigation.navigate("SearchStationary",{tex: {text}});}}>
